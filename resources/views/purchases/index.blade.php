...
@section('content')
  <section class="dashboard-main">
    ...
    <form class="filter-form" action="{{ route('purchases.index') }}" method="get">
      ...
      <div class="form-group"><label class="form-label" for="status">状態</label><select id="status" class="form-control" name="status"><option value="">すべて</option><option value="draft" @selected(request('status') === 'draft')>下書き</option><option value="confirmed" @selected(request('status') === 'confirmed')>確定済み</option></select></div>
    ...
    <div class="table-wrapper"><table class="data-table"><thead><tr><th>伝票番号</th><th>仕入日</th><th>仕入先</th><th>合計金額</th><th>状態</th><th>登録者</th><th></th></tr></thead><tbody>
      @forelse($purchases as $purchase)<tr><td>{{ $purchase->purchase_number }}</td><td>{{ $purchase->purchase_date->format('Y/m/d') }}</td><td>{{ $purchase->supplier->name }}</td><td>{{ number_format((float) $purchase->total_amount, 2) }} 円</td><td>{{ $purchase->isDraft() ? '下書き' : '確定済み' }}</td><td>{{ $purchase->creator->name }}</td><td><a class="action-button" href="{{ route('purchases.show', $purchase) }}">詳細</a></td></tr>
