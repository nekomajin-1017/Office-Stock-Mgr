...
class SaleRegistrationTest extends TestCase
{
    ...
    {
        [$user, $customer, $product] = $this->candidates();
        $inactive = $this->product($product->category, 'P2', '無効商品', false);
    ...
    {
        [$user, $customer, $product] = $this->candidates();
        $second = $this->product($product->category, 'P2', '商品2', true);
        Stock::create(['product_id' => $second->id, 'quantity' => 10, 'average_cost' => 0]);
        $payload = $this->payload($customer, $product, 2, 150);
        $payload['items'][] = ['product_id' => $second->id, 'quantity' => 3, 'unit_price' => 200];
    ...
    {
        [$user, $customer, $product] = $this->candidates();
        $this->actingAs($user)->post(route('sales.store'), $this->payload($customer, $product, 5));
    ...
    public function test_transaction_rolls_back_when_item_creation_fails(): void
    {
        ...
        try {
            $this->actingAs($user)->post(route('sales.store'), $this->payload($customer, $product));
            $this->fail('例外が必要です。');
    ...
    private function candidates(): array
    {
        ...
        $product = $this->product($category, 'P1', '有効商品', true);
    ...
    {
        return ['customer_id' => $customer->id, 'sale_date' => '2026-07-28', 'items' => [['product_id' => $product->id, 'quantity' => $quantity, 'unit_price' => $price]]];
    }
}
